...
func (app *api) translateText(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Printf("phrase: %s", text)
	textHint := makeHintString(text)
	fmt.Printf("phrase_hint: %s", textHint)
	fmt.Printf("translates: %s", resp[0].Text)
	translateHint := makeHintString(resp[0].Text)
	fmt.Printf("translateHint: %s", translateHint)
