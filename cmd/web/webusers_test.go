...
func (suite *WebNoLoginTestSuite) TestUserSignupPost() {
	...
	tests := []struct {
	...
	}{
		{
			...
			userLanguageId: test.ValidLanguageId,
		...
		},
		{
			...
			userEmail:      "shortpassword@email.com",
			userLanguageId: test.ValidLanguageId,
			userPassword:   "pa$$",
			csrfToken:      suite.validCSRFToken,
			wantCode:       http.StatusUnprocessableEntity,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			form.Add("language_id", strconv.Itoa(tt.userLanguageId))
...
func (suite *WebTestSuite) TestAccountLanguageUpdatePost() {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			form := url.Values{}
			form.Add("language", strconv.Itoa(tt.userLanguageId))
			form.Add("csrf_token", tt.csrfToken)
