...
sed '/\[talkliketv\]/{n;s/.*/34.168.230.173/g;}' ../ansible/inventory.txt > output.file
mv output.file ../ansible/inventory.txt

# uncomment first sed for linux, second for mac. replaces CLOUD_HOST_IP in .envrc file
# sed -i 's/^export CLOUD_HOST_IP=.*$/export CLOUD_HOST_IP=34.168.230.173/' ../.envrc
sed -i '' -e 's/^export CLOUD_HOST_IP=.*$/export CLOUD_HOST_IP=34.168.230.173/' ../.envrc

# run ansible playbook locally
# shellcheck disable=SC2154
ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -i '../ansible/inventory.txt' \
  -e 'gcp_public_ip=34.168.230.173' \
